...
```
Bu Docker Compose dosyası, bir adet backend, Prometheus ve Grafana sunucusunu başlatacaktır. Bu sistemler otomatik olarak birbirine bağlıdır. Backend'in /metrics alanı altında Prometheus için metrik çıktıları bulunmaktadır. Ayrıca, yaml klasörü altında Prometheus, Grafana ve Golang metriklerini takip eden bir dashboard ayakta olacaktır.